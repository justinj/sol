...
var stateIndexSolved = function stateIndexSolved(state) {
...
var solveAtDepth = function solveAtDepth(state, depth, puz, lastMove, ttable, ptable, justOne) {
...
var solve = function solve(opts) {
  (0, _lodash.defaults)(opts, DEFAULTS);
  var puz = opts.puz;
  var state = opts.state;
  var minDepth = opts.minDepth;
  var justOne = opts.justOne;

  var depth = minDepth;
  var componentNames = Object.keys(puz.components);
  var ttable = puz.getTransitionTables();
  var ptable = puz.getPruningTables();
  var curState = componentNames.map(function (name) {
    return puz.components[name].index(state[name]);
  });
  ...
  var strs = solutions.map(function (solution) {
    return solution.map(function (i) {
      return puz.moves[i];
    }).join(" ");
  });
  if (justOne) {
