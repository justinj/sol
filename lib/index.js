...
var solveAtDepth = function(state, depth, puz, lastMove, ttable) {
  ...
  if (depth === 0 && stateIndexSolved(state)) {
    ...
    for (var i = 0; i < followups[lastMove].length; i++) {
      ...
      var result = solveAtDepth(newState, depth - 1, puz, move, ttable);
      if (result !== false) {
...
var solve = function(puz, state) {
  ...
  while (!result && depth <= 11) {
    result = solveAtDepth(curIndex, depth, puz, -1, ttable);
    depth += 1;
  }
