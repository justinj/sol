...
function frequencies(ary) {
  let res = {};
  ary.forEach(x => res[x] = (res[x] || 0) + 1);
  return res;
}

export function countRepeatedPerms(perm) {
  let typesRemaining = frequencies(perm);
...
export function repeatedPermToIndex(perm) {
  ...
  let withoutCombCounts = countRepeatedPerms(withoutComb);
